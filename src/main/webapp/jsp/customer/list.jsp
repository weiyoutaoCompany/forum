...
<table class="table table-bordered table-hover  table-striped">
    ...
    <c:forEach var="customer" items="${requestScope.list}">
        <tr>
            ...
            <td>${customer.name}</td>
            <td style="width:20%;"><a href="<%=basePath %>backstage/adminmanage/toUpdateAdmin?id=${customer.id}" class="btn btn-info btn-xs">修改</a>
                <a name="deleteAdmin" class="btn btn-info btn-xs" href="<%=basePath %>backstage/adminmanage/doDeleteAdmin?id=${customer.id}">删除</a>
            </td>
        </tr>
    </c:forEach>
    </tbody>
